...
보존한다. policy(LLM)는 **무엇을 남길지** 의미 결정만(`keep`/`check`/`recall`), 엔진이
**우선순위 정렬 · 중복제거 · 정원(cap) · 확인 기록 · compact 렌더** 를 관리한다.
엔진 철학 유지: **도메인 agnostic · opt-in · 무하드코딩 · Progressive Disclosure 정합**.
검색/임베딩 결선은 엔진이 아니라 이식측 소유(여기는 generic 자료구조 + 통로만).

- **`memory/recall.py` — Recall Workspace (공개 API).**
  - `Priority`(critical/high/normal/low) · `RecallItem`(content/source/priority/score/
    checked/note/turn/fingerprint) · `RecallSet`.
  - `RecallSet`: 우선순위 태깅 + **내용지문 dedup**(같은 id/본문 중복 누적 차단) +
    **cap eviction**(최저 우선순위→오래된 순, cap 은 파라미터) + `check`(확인 기록) +
    `ranked()` + `render()`(step-out compact 뷰) + JSON 직렬화(SessionStore 세션 인계).
- **`memory/dedupe.py` — `content_fingerprint` / `dedupe`** 일반 유틸 (관측 중복 제거).
- **빌트인 툴 `keep` / `check` / `recall`** (opt-in — s04 `builtin_tools` 에 명시될 때만 등록).
  전체 본문은 `pd_stores["recall"]` 로 보존되어 `fetch_pd(kind="recall", id=...)` 로
  **step-in**, `recall` 로 **step-out**.
- **`tool_diversity` Guard (신규 내장).** 동일 (tool, args) 호출이 `max_repeats` 회 이상
  반복되면 PRE_TOOL 차단 + "접근을 바꾸거나 멈추라" 교정(무의미한 루프 방지). `xgen_harness.guards` 등록.
- **공개 API**: `xgen_harness.{Priority, RecallItem, RecallSet}` + `memory` 재export.
- 코어 영향 0(미사용 하네스): s04 recall 빌트인은 opt-in, guard 는 명시 활성 시만.
  state.py / 기존 stage 동작 불변. 테스트 225 green.

## v1.19.0 (2026-06-15) — 작업기억 보존소 1차 (v1.19.1 에서 공개 API 명칭 확정)

내부 작업기억 + 반복호출 방지 Guard 1차 도입. 공개 API 명칭은 v1.19.1 에서 `Recall*` 로 확정.
...
### Fixed
- **합성 보강 호출 (도구 호출 후 final answer)** 에 SYNTHESIS MODE 지시 박음.
  도구 결과 받은 후 LLM 이 "...정보를 알려주시면 진행하겠습니다" / "...파일 목록을
  가져올까요?" 같은 사용자 확인 톤을 반복하던 회귀 (2026-05-16 라이브 로그
  적발). 보강 호출 시 기존 system_prompt 끝에 "도구 결과만으로 최종 답변 작성,
  추가 확인 요청 금지, 새 도구 제안 금지" 명시 지시 append.
